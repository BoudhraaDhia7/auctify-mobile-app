...
import { searchPhoneByNumber, sendSoldeToUser } from '../../../apis/actions';
...
const SoldeSend = ({ close } : Props) => {
    ...
    const sendSolde = async() => {
        setIsSending(true);
        try {
            if (phoneInfo?.id) {
                const res = await sendSoldeToUser( profile.connexionInfo.idClient, profile.connexionInfo.idClient, phoneInfo?.id, -amount );
                if (res?.message == "DANNOS_SENDED") {
        ...
        return(
            <>
                ...
                { isSending ? <View style={styles.btConfirm}>
                    <ActivityIndicator size={'large'} color={"#FFF"} />
                </View> : <TouchableOpacity style={styles.btConfirm} onPress={() => sendSolde()}>
    ...
    return(
        ...
        >
            <View style={styles.content}>
                <View style={styles.sectionTitle}><Text style={styles.sectionTitleLabel}>Envoi Solde</Text></View>
